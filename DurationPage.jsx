...
export async function getPool(regionKey, dbName) {
  ...
  // If there's a cached pool, check if it's connected — otherwise re-create
  if (pools[cacheKey]) {
    try {
      // ensure connection is alive by a quick ping
      // some versions of mssql expose .connected or .connectedState; safest is a test query
      await pools[cacheKey].request().query('SELECT 1');
      return pools[cacheKey];
    } catch (err) {
      // stale/broken pool — remove and recreate
      try { pools[cacheKey].close && pools[cacheKey].close(); } catch (e) { /* ignore */ }
 ...
 *
 * Note: this query creates a #CombinedEmployeeData temp table. Because connections are pooled,
 * we must DROP any existing temp table at the start and at end to avoid "already exists" or stale data.
 */
export async function timeDurationReport(region, { partition = 'Default', startDate }) {
  ...
  const query = `
-- ensure no leftover temp table in this session
IF OBJECT_ID('tempdb..#CombinedEmployeeData') IS NOT NULL DROP TABLE #CombinedEmployeeData;
...
WITH DailyDurations AS (
    SELECT 
        ...
        RIGHT('00' + CAST((DATEDIFF(MINUTE, MIN(LocaleMessageTime), MAX(LocaleMessageTime)) / 60) AS VARCHAR(10)), 2)
        + ':' +
        RIGHT('00' + CAST((DATEDIFF(MINUTE, MIN(LocaleMessageTime), MAX(LocaleMessageTime)) % 60) AS VARCHAR(10)), 2) AS DurationHHMM,
        CASE 
...
/**
 * eurocAdmitRejectionReport - single working DB (kept as before, just ensure cleanup already included)
 */
...
// ... (static constants and helper functions unchanged) ...
...
// ---------- New helper: sanitize long/illegal strings for Excel ----------
function sanitizeCellValue(v, maxLen = 32000) {
  if (v === null || v === undefined) return '';
  let s = (typeof v === 'string') ? v : String(v);
  // remove control chars except tab(9), newline(10), carriage(13)
  s = s.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F]/g, ' ');
  if (s.length > maxLen) {
    // keep end short and indicate truncation
    s = s.slice(0, maxLen - 12) + '...[truncated]';
  }
  return s;
}
...
export default function ReportsPage() {
  ...
  const generateTimeDurationExcel = async data => {
    ...
    data.forEach((r, idx) => {
      ...
      const row = ws.addRow([
        idx + 1,
        sanitizeCellValue(r.ObjectName1),
        sanitizeCellValue(r.EmployeeID),
        sanitizeCellValue(r.PersonnelType),
        sanitizeCellValue(r.PartitionName2),
        sanitizeCellValue(r.text5),
        r.YearNumber,
        r.WeekNumber,
        date,
        first,
        last,
        sanitizeCellValue(r.DurationHHMM),
        sanitizeCellValue(r.TimeDiffCategory),
        r.DaysPresentInWeek,
        r.ViolationDaysInWeek,
        r.CleanDaysInWeek,
        sanitizeCellValue(r.Defaulter)
      ]);
...
const generateRejectionExcel = async data => {
  ...
  data.forEach((r, idx) => {
    ...
    const rowVals = [
      null, // column A blank
      idx + 1,
      sanitizeCellValue(dateStr),
      sanitizeCellValue(timeStr),
      sanitizeCellValue(r.ObjectName1 || ''),
      sanitizeCellValue(r.EmployeeID || ''),
      sanitizeCellValue(r.CardNumber || ''),
      sanitizeCellValue(r.Rejection_Type || ''),
      sanitizeCellValue(r.Door || r.ObjectName2 || ''),
      sanitizeCellValue(r.location || ''),
      sanitizeCellValue(r.Direction || r.Swipe || '')
    ];
  ...
  countEntries.forEach(([type, cnt]) => {
    ws.getCell(cur, summaryStartCol).value = sanitizeCellValue(type);
 ...
 const generateRawExcel = async (rows, opts = {}) => {
    // rows: array of raw rows
    // opts: { region, location, rawSearch, from, to, rawAdmitFilter }

    // group by month (derived from DateOnly or LocaleMessageTime)
    const groups = {};
    (rows || []).forEach(r => {
      const iso = getServerISO(r) || (r.DateOnly ? (String(r.DateOnly).slice(0,10) + 'T00:00:00.000') : '');
      let yyyy = 'unknown';
      let mm = '00';
      if (iso && iso.length >= 7) {
        yyyy = iso.slice(0,4);
    ...
    // If there are no rows at all, create an empty sheet with header only
    ...
    // helper to create a single worksheet and fill rows list
    const createSheetWithRows = (sheetName, rowsForSheet) => {
      const safeSheetName = (sheetName || 'Raw').substring(0, 31); // Excel limit
      const ws = wb.addWorksheet(safeSheetName);
      hideGridLines(ws);

      const hdr = ws.addRow(headers.map(h => sanitizeCellValue(h)));
      applyHeaderStyles(hdr);

      if (rowsForSheet && rowsForSheet.length) {
        rowsForSheet.forEach(r => {
          ...
          const fullObj = {
            LocaleMessageTime: iso ? `${dateStr} ${timeStr}` : (r.LocaleMessageTime || ''),
            DateOnly: r.DateOnly ? formatDateFromServerISO(r.DateOnly) : '',
            Swipe_Time: r.Swipe_Time ? timeStr : '',
            EmployeeID: r.EmployeeID || '',
            ObjectName1: r.ObjectName1 || '',
            PersonnelType: r.PersonnelType || '',
            location: r.location || '',
          ...
          const rowVals = headers.map(h => sanitizeCellValue(fullObj[h] !== undefined && fullObj[h] !== null ? fullObj[h] : ''));
    ...
    // If zero rows: create one sheet and return
    const groupKeys = Object.keys(groups).sort();
    if (groupKeys.length === 0) {
      createSheetWithRows('Raw', []);
    } else {
      // chunk size for safety (ExcelJS and Excel can struggle when sheet is HUGE)
      const CHUNK_SIZE = 50000;
      for (const key of groupKeys) {
        const groupRows = groups[key];
        if (!groupRows || groupRows.length === 0) continue;
        // create base sheet name like Raw_2025_09 or Raw_Sep_2025
        const [yyyy, mm] = key.split('-');
        const mmIdx = parseInt(mm || '0', 10) - 1;
        const monthName = (mmIdx >= 0 && mmIdx < 12) ? MONTH_ABBR[mmIdx] : mm;
        const baseName = `Raw_${monthName}_${yyyy}`;

        if (groupRows.length <= CHUNK_SIZE) {
          createSheetWithRows(baseName, groupRows);
        } else {
          // chunk into multiple sheets
          let sliceIndex = 0;
          let part = 1;
          while (sliceIndex < groupRows.length) {
            const chunk = groupRows.slice(sliceIndex, sliceIndex + CHUNK_SIZE);
            createSheetWithRows(`${baseName}_${part}`, chunk);
            part++;
    ...
    try {
      ...
      if (tab === 1) {
        ...
        try {
          ...
          // Use allSettled so a failure in one region doesn't drop all others
          ...
          if (failures.length && successes.length === 0) {
            // if none succeeded, throw the first failure to show meaningful error
            throw failures[0].reason;
          }

          rowsSrcCombined = successes.flatMap(r => (r.data && (r.data.data || r.data)) || []);
        } catch (err) {
      ...
      if (tab === 3) {
        ...
        // ---------- EUROC Admit-Reject generation (updated formatting) ----------
        {
          ...
          function createFormattedSheet(sheetName, titleText, headers, rowsData) {
            ...
            const headerRow = ws.addRow([null, ...headers.map(h => sanitizeCellValue(h))]);
            headerRow.height = 20;
            headerRow.eachCell((cell, colNumber) => {
            ...
            (rowsData || []).forEach((r, idx) => {
              ...
              const row = ws.addRow([null, ...vals.map(v => sanitizeCellValue(v))]);
          ...
          (function addSummaryToRejectionSheet() {
            ...
            entries.forEach(([type, cnt]) => {
              ws2.getCell(cur, summaryStartCol).value = sanitizeCellValue(type);
        ...
        if (region === 'laca') {
          ...
          if (!location) url = 'http://10.199.22.57:3000/history';
            ...
            const APAC_URLS = {
              ...
              'MY.Kuala Lumpur': 'http://10.199.22.57:3000/partition/MY.Kuala%20Lumpur/history',
